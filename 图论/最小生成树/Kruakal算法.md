...
那么**克鲁斯卡尔**的思想是:先把给出的边按照权值排序，然后依次从权值小的向权值大的进行加边，在加边的时候利用**并查集**进行判断当前两个顶点是否连通，如果没有连通就加入进去，当加入的次数等于n-1(顶点数-1)的时候停止，当前的权值之和就是最小生成树的最小花费。其时间复杂度主要在边排序上，约为 $O(m\log m)$（$m$ 为边数），并查集判断近乎线性，因此 Kruskal 在稀疏图上更具优势。

例题:

[HDU1233 还是畅通工程(最小生成树模板题，Prim,kruskal算法)](http://blog.csdn.net/riba2534/article/details/60318424)
