...
这个算法的思想是，我们给广搜的网络标一个高度，每一层一个高度，最后一层的高度是0，然后按照广搜的层次，到起点依次递增。然后搜索的时候直接按照标签的高度从高到低找，这样可以加快效率，当走的点的标签高度走不动的时候，我们采取**重贴标签**的策略，令**当前节点的高度=所有邻接点高度的最小值+1**，如果没有邻接边，则令**当前节点的高度=节点数**（注意：一定要保证容量>流量）,然后不断的进行贴标签，重贴标签的过程，累积**可增广量**，最后的值就是**最大流**
