...
我们利用染色法来判断一个图是不是二分图，先找一个点为起点，然后把与它相连的点的颜色都变成与它相反的颜色，然后把这些点加入到队列中，遍历到已染色的相邻点时，若它与当前点颜色相反则合法、跳过，若颜色相同则说明存在奇环、这个图不是二分图

时间复杂度 $O(V+E)$，空间复杂度 $O(V+E)$。下面代码中的宏 `mem` 即 `#define mem(a,b) memset(a,b,sizeof(a))`。
