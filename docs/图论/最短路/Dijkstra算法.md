...
一种单源最短路径算法，已知顶点和边，然后可以求出图中一点和另一点之间的最短距离。思想是:利用一个dis[i]数组来记录，从顶点1到其他各个点的最短距离，刚开始的时候全部初始化为INF，从顶点1开始找它的出边里面最小的，然后把这个顶点标记，然后松弛dis数组，之后找出这个点的所有出边里面最小的，然后标记，更新，一直到最后,时间复杂度是 $O(n^2)$

和 Prim 算法的思想差不多，都是松弛，找到第一个点，然后进行对于dis的松弛，一般有**邻接矩阵**和**邻接表**两种写法
...
2. 使用免费机会的时候:
   ...
   如果 到点u的时候使用了k次机会的花费dis[u][k]<到v点的时候使用了k+1次机会的边权dis[v][k+1]
   	...
   	加入堆q.push(node(v,k+1,dis[v][k+1]))
   ```
