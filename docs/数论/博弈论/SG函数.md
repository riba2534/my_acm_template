...
> x=1时，可以取走1-f{1}个石子，剩余{0}个，后继SG值集合为{sg[0]}={0}，故sg[1]=mex{0}=1;
>
> x=2时，可以取走2-f{1}个石子，剩余{1}个，后继SG值集合为{sg[1]}={1}，故sg[2]=mex{1}=0；
>
> x=3时，可以取走3-f{1,3}个石子，剩余{2,0}个，后继SG值集合为{sg[2],sg[0]}={0,0}，故sg[3]=mex{0,0}=1;
>
> x=4时，可以取走4-f{1,3,4}个石子，剩余{3,1,0}个，后继SG值集合为{sg[3],sg[1],sg[0]}={1,1,0}，故sg[4]=mex{1,1,0}=2;
>
> x=5时，可以取走5-f{1,3,4}个石子，剩余{4,2,1}个，后继SG值集合为{sg[4],sg[2],sg[1]}={2,0,1}，故sg[5]=mex{2,0,1}=3；
> 以此类推.....
...
```cpp
/*
f[]:可以取走的石子个数,必须要升序(一旦 f[j] > i 即停止累计后继)
sg[]:0~n的sg的值
vis[]:用来求解mex{}
ps:f数组的下标，从1开始
...
复杂度：打表法 get_sg 时间复杂度 $O(n\cdot m)$（$n$ 为石子上限，$m=|f|$），DFS 记忆化为 $O(\text{状态数}\cdot m)$；空间均为 $O(n)$。
