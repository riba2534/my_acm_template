...
-   我们的线段树就是为了维护矩形的长，我们给每一个矩形的上下边进行标记，下面的边标记为1，上面的边标记为-1，每遇到一个矩形时，我们知道了标记为1的边，我们就加进来这一条矩形的长，等到扫描到-1时，证明这一条边需要删除，就删去，利用1和-1可以轻松得到这种状态。
...
-   复杂度：排序 $O(n\log n)$，扫描配合线段树更新 $O(n\log n)$，总体 $O(n\log n)$，空间 $O(n)$（其中 $n$ 为矩形条数对应的边数）。
