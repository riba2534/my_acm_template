...
时间复杂度 $O(n\log n)$（排序为主导，置换环遍历为 $O(n)$）。
...
一：
对于每一个环的贪心思路就是，找到这个环中最小的那个点，也就是6，然后从6开始进行交换，6和9交换，可以使9到对应的位置，花费为`6+9=15`,然后6和7交换，花费为`6+7=13`，最后等到交换完毕，那最后的答案是什么呢？就是：
$$
(6+9)+(6+7)+(6+8)=(6+7+8+9)+6\times 2=30+12=42
$$
...
$$
(1+6)+(1+9)+(1+7)+(1+8)+(1+6)=(6+8+7+9)+1\times 5+6=41
$$
...
时间复杂度 $O(n\log n)$（排序与 `map<ll,ll>` 离散化定位均为 $O(n\log n)$，置换环遍历为 $O(n)$）。