...
比如  1 2 1,3 4 1
这时候应该 输出1 2，所以我们在更新区间的时候要给左区间加上1.剩下的就是在询问的时候，用一个值来代表当前的颜色，只有当颜色不同时，对应记录各颜色区间个数的数组要 +1
...
- pow x：表示把x以及x的子树的灯变为相反状态，其实也就是异或操作
