...
1.  如果当前的cnt>1的时候，说明覆盖过两次，直接计算
2.  如果当前左右区间相等，那么s就肯定为0
3.  如果当前cnt==1，那么就说明这个区间已经被完全覆盖过了，我们要使它覆盖两次，只需要在它的左右儿子里面找到覆盖过的区间，就可以使它变成覆盖两次的
4.  对于其他情况，由所有儿子的值求和得到

设事件数(矩形边)为 $n$:排序 $O(n\log n)$,每条扫描线更新线段树 $O(\log n)$,整体时间复杂度 $O(n\log n)$,空间复杂度 $O(n)$.
