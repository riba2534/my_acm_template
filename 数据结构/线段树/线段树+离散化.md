...
在一面很长很长的墙上贴海报，每张海报的高度一样，唯一的区别是贴海报的位置不同，问最后能看见几张海报。

因为墙的长度长，直接线段树可能超时，所以要进行离散化处理，将离散后的坐标映射到线段树中，并不影响最后所求的海报长度。在这道题中，因为海报是按照`段`来算的，而不是`点`，所以可能会出现离散化之后有缝隙的问题,比如:`[1,10]、[1,3]、[6,10]`离散化之后是`[1,4]、[1,2]、[3,4]`这样会导致2和3之间的缝隙丢失导致错误，所以在去重完毕后，进行一个处理,如果相邻数字间距大于1的话,在其中加上任意一个数字。这样会把原来的缝隙留出来。
