...
第k小:如`1 2 3 3 3 4 4 5`第4小的数是3，第5小的数还是3；第1小是1，第8小是5
...
int query(int k, int l, int r, int rt) //求整个线段树中第k小(升序第k个)
{
    if (l == r)
        return l;
    int m = (l + r) >> 1;
...
int main()
{
    ...
    while (~scanf("%d", &n))
    {
        ...
        while (n--)
        {
            ...
            {
                scanf("%d%d", &x, &k);
                int ans = Rank(x, 1, N, 1); //查找<=x的数的个数
                if (sum[1] < ans + k)
                    puts("Not Find!");
                else
                    printf("%d\n", query(ans + k, 1, N, 1));
...
复杂度：建树 $O(N)$，插入/删除(`update`)、查询排名(`Rank`)、求第k小(`query`)单次操作均为 $O(\log N)$。