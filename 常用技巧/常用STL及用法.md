...
3. `upper_bound`它试图在已排序的[first,last)中寻找value，返回可安插value的最后一个合适的位置。如果value存在，lower_bound 返回的是指向该元素的iterator。相较之下upper_bound并不这么做，它返回value可被安插的最后一个合适位置。如果value存在，那么它返回的iterator将指向value的下一个位置，而非value自身。即返回第一个严格大于 value 的元素位置。
...
c.end()             返回指向容器最后一个元素之后(尾后)的迭代器，而非最后一个元素  
...
set_difference() 的返回值是一个指向result序列末尾的迭代器. 
...
>   1.  vector 就 是 一 个 智 能型 的 一 维数 组 , 能 够 自 动 调整 存 储 空间 。 vector 还 优 化 了 `vector<bool>` 的存储方式,用每一位来存储一个 bool。但是由于信息学竞赛中数据的规模一
...
> 注意：__builtin_clz/__builtin_ctz 在参数为 0 时结果未定义(undefined)，使用前需先判断 x 是否为 0，这是竞赛中常见的踩坑点。