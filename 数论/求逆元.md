...
1. **费马小定理**，有$n^{p-1} \equiv 1 \pmod p$。
   因此$\frac{1}{n} \equiv n^{p-2} \pmod p$

2. 方程$ax \equiv 1 \pmod p$的解被称为a关于模p的逆元。**当p为素数且$\gcd(a,p)=1$**（此时必有解且唯一）时，由费马小定理可得$x \equiv a^{p-2} \pmod p$。

   例如: $x=\frac{5}{2}\bmod p$可以转化为$x=5*2^{p-2}\%p$
...
这个算法可以在$O(n)$的时间复杂度内求出n以内所有数的逆元，不过需要注意的是p必须为奇质数。
