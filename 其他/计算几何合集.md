...
/********* 点是否在一条线段上 **********/
//此处用 Dot<0（严格小于），不含线段端点；下方 ponls 含端点，两者语义不同
bool OnSegment(Point p,Point a1,Point a2)
{
    return dcmp(Cross(a1-p,a2-p)) == 0 && dcmp(Dot(a1-p,a2-p)) <0 ;
}
/*********    求多边形面积 **********/
//凸或凹的简单多边形均可，顶点需按顺时针或逆时针顺序有序存储
...
int graham(Point p[],int n,int pl[])    //点集，点的个数，凸包顶点集（此处为卷包裹法/Jarvis march 实现，非经典 Graham 扫描，复杂度 O(nH)）
{
...
double graham(Point p[],int n)    //点集和点的个数（此处为卷包裹法/Jarvis march 求凸包周长，非经典 Graham 扫描，复杂度 O(nH)） 
{
    int pl[10005];
    //找到纵坐标（y）最小的那个点，作第一个点 
    int t = 1;
...
double getPS(Point p[],int n)    //返回多边形面积。必须确保 n>=3，且顶点按顺时针或逆时针顺序存储（凸或凹的简单/非自交多边形均可，自交多边形结果无意义） 
{
    double sumS=0;
    for(int i=1;i<=n-1;i++)
...
1，建立x，y，z直角坐标系。设A、B、C三点的坐标分别为(a1,b1,c1),(a2,b2,c2),(a3,b3,c3),四面体O-ABC的六条棱长分别为l，m，n，p，q，r；
